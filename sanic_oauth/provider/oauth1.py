...
from sanic.app import url_for, Sanic._run_response_middleware
...
class OAuth1Provider(object):
    ...
    And now you can protect the resource with realms::
        ...
        def user():
            return response.json(json.dumps(request.oauth.user))
    """
        ...
        for setting limitation on the client request::
            ...
            def limit_client_request():
                ...
                if over_limit(client):
                    return ServerError("Forbidden", status_code=403)
    ...
    def require_oauth(self, *realms, **kwargs):
        ...
        def wrapper(f):
            ...
            def decorated(*args, **kwargs):
                ...
                if not valid:
                    return ServerError("Unauthorized", status_code=401)
